...
**IN PROGRESS / closeout deferred.** This document defines the M4 milestone. M4-00 is complete; M4-01 (Android string-resource and Nepali localization foundation) is complete and merged into `main` via pull request #8; M4-02 (Nepal-oriented currency, number, date and time presentation) is complete and merged into `main` via pull request #10; M4-03 (first-run and daily transaction-entry usability) is complete and merged into `main` via pull request #11. M4-04 (physical-device and guided farmer-pilot validation) is **complete**: executed on the Pixel 7a (M4-04-01/02/03) and the Moto Edge 60 Fusion (M4-04-04 upgrade gate, M4-04-05 second physical-device gate). M4-05 product corrections are recorded, but release-candidate closeout remains deferred until the remaining manual gates and exact-candidate upgrade gate in `docs/release/V0.2.0_RELEASE_CHECKLIST.md` are completed. See `docs/validation/M4_01_LOCALIZATION_VALIDATION.md`, `docs/validation/M4_02_PRESENTATION_VALIDATION.md`, `docs/validation/M4_03_DAILY_ENTRY_USABILITY_VALIDATION.md`, and `docs/validation/M4_04_GATE_DISPOSITION.md` for the validation records.

Current status:

```text
M4 — IN PROGRESS / CLOSEOUT DEFERRED
M4-00 — COMPLETE
M4-01 — COMPLETE
M4-02 — COMPLETE
M4-03 — COMPLETE
M4-04 — COMPLETE
M4-05 — CORRECTIONS RECORDED; RELEASE-CANDIDATE CLOSEOUT DEFERRED
```
...
- **M4-05 — Defect correction, release-candidate validation and closeout.** Correct defects found in M4-04 (and architecture findings from the pre-RC design review), validate a signed `v0.2.0` release candidate, and close out the milestone. Status: IN PROGRESS. In scope: D001 (Transport category), D002 (past-date picker), **F001 (currency ownership correction)**, complete regression, signed `v0.2.0` RC, independent signer/hash/version verification, `v0.1.0 → v0.2.0` preservation/upgrade proof, and the **residual M4 exit-gate item: older/lower-resource physical-device coverage — NOT YET PROVEN ON PHYSICAL HARDWARE** (Moto Edge 6 Fusion is the second physical device at API 36; the API-26 emulator stays supplemental/non-physical). D001 (dedicated `TRANSPORT` expense category) is implemented and validated — see `docs/validation/M4_05_D001_TRANSPORT_CATEGORY.md`. D002 (date/time picker) was evaluated on-device and dispositioned **DEFERRED / ACCEPTED** with no production change — see `docs/validation/M4_05_D002_DATETIME_PICKER_DISPOSITION.md`. F001 (transaction-level currency ownership conflicts with the farm-wide single-currency invariant, discovered in the pre-RC design review) is corrected: currency moves to farm-level `FarmState.currencyCode`, persistence moves to schema 3 with schema-2/legacy migration, the transaction editor drops all currency machinery, and a temporary Farm Tools currency control (locked once transactions exist) provides the M4-05 bridge to the M5-00 Settings destination — see `docs/validation/M4_05_F001_CURRENCY_OWNERSHIP.md`.
