...
- Manual/device validation **completed at the v0.2.0 candidate freeze** (RC-01, Moto API-36 physical): M5-05 Financial Overview (period switching with period-distinguishing data across all four sections, empty states, recreation restore), M6 Party Hisab (party/period switching with an out-of-window purchase so THIS_MONTH/ALL_TIME differ, recreation restore of party + period), and the M6.3 calculator battery (all arithmetic operations, divide-by-zero, blank input, profit/loss/margin/markup, negative guards, simple interest asserting the complete labeled result, Hill/Terai land conversions) — covered by `FarmOverviewAndHisabDeviceTest` and `KisanToolboxDeviceBatteryTest`, with the connected instrumentation suite at **96/96** (86 baseline + 10 new). Overflow handling and Nepali rendering remain unit-covered. See `docs/release/V0.2.0_RELEASE_CHECKLIST.md` for the release-candidate gate record.
...
- Device validation already completed: M6.4 farm-planning calculators (keyboard, selector, small-screen, long-result, Nepali rendering, and persistence isolation) on an API-36 physical device and an API-26 emulator (`docs/milestones/M6_4_FARM_INPUT_CALCULATORS.md`); M6.4.1 shell/system-bar insets including the full connected instrumentation suite (**96/96** on the API-36 physical device at RC-01 head; the API-26 arm64 small-screen emulator at `1080×1920 @ 420 dpi ≈ 411 dp` shows 3–5 Espresso coordinate/geometry flakes per run — including the two documented back-navigation coordinate flakes reproduced on the pre-change baseline plus small-screen geometry items like `partyEditorDelete…` and an intermittent `landConversions…` — all test-robustness, not product defects, none reproducing on the API-36 physical device, `docs/milestones/M6_4_1_SHELL_SYSTEM_BAR_INSETS.md` and checklist Section F).
