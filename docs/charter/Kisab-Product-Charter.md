...
- **M6.4** — Farm Input Calculators: temporary farmer-entered seed, fertilizer, feed, milk, and crop-yield quantity/cost/revenue projections in Hisab, without agronomy recommendations, persistence, or accounting side effects. **Status: COMPLETE (automated); manual/device validation deferred** (see `docs/milestones/M6_4_FARM_INPUT_CALCULATORS.md`).
