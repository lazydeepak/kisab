...
| Account setup / email login | Design resolved: **Email OTP** selected, lifecycle specified in [ADR-0004](decisions/ADR-0004-email-otp-method-and-lifecycle.md). Implementation not started: add `AuthProvider.EMAIL`, `AccountApi.requestEmailOtp`/`verifyEmailOtpAndEstablish` (+`OTP_EXPIRED`/`OTP_RATE_LIMITED`), email+OTP UI, wiring to `OnlineAccountService`; testable against `FakeAccountApi` without a real backend. Then real backend/provider integration as separate follow-up. |
