...
| --- | --- |
| Global styling audit | Danger-button wiring complete with device acceptance; light-mode captures and contrast review recorded in [validation](validation/STYLING_DANGER_BUTTON_WIRING_VALIDATION.md). Contrast finding: disabled-over-light `#C62828@25%` + white label = 1.51:1 (defect for full audit). Remaining: full screen/dialog sweep for shared colors/dimensions, light/dark pressed/disabled states, touch targets, text scaling, insets, keyboard overlap on other dialogs/sheets; pressed-state label contrast (`#FF5252`, 3.19:1) and disabled-state rework. |
