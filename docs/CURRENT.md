...
| --- | --- |
| Global styling audit | Complete for this cycle: danger-button wiring, full-surface sweep, and label-contrast rework all recorded in [validation](validation/STYLING_DANGER_BUTTON_WIRING_VALIDATION.md). Two accepted MINOR items moved to [backlog](BACKLOG.md) (S1 Add-Farm IME dip, S2 padding drift). Re-open only for a future dedicated styling pass or farmer-reported issue. |
| English/Nepali validation | Automated part rechecked 2026-09-12: `LocalizationParityTest` PASS; lint found 3 `PluralsCandidate` defects ("%1$d days" → plurals, en + ne, call sites updated) now fixed and recorded in [validation](validation/LOCALIZATION_PLURAL_FIX_VALIDATION.md). Remaining: human bilingual review must validate meaning, terminology, dates/numbers, truncation, real workflows, plus the logged open items (shared Nepali warning/critical title, four benign `SetTextI18n` sites). |
| Account setup / email login | **Implementation complete (PR #64, branch `feat/email-otp-account-flow`, merge pending maintainer acceptance).** ADR-0004 adopted; contract (`AuthProvider.EMAIL`, `AccountApi` OTP methods + failure kinds), `FakeAccountApi` OTP lifecycle, `EmailOtpFlow`, `OnlineAccountService.persistEstablishment`, EN/NE strings, and FarmActivity UI (button + dialogs) all merged to branch. `verifyLocal` BUILD SUCCESSFUL; 638 unit tests PASS. Device smoke on Moto Edge 60 Fusion (`/tmp/account_signin.png`, `/tmp/account_otp_dialog.png`). Human bilingual review open (Nepali meaning, shared warning/critical title, 4 benign `SetTextI18n`). Real backend/provider integration remains separate follow-up. Validation record: `docs/validation/ACCOUNT_EMAIL_OTP_FLOW_VALIDATION.md`. |
