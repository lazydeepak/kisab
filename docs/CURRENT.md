...
| --- | --- |
| Global styling audit | Danger-button wiring is complete with device acceptance (see [validation](validation/STYLING_DANGER_BUTTON_WIRING_VALIDATION.md)). Remaining: audit all screens, dialogs, buttons and empty states for shared colors/dimensions, light/dark pressed/disabled states, contrast (incl. pressed `#FF5252` + white label), touch targets, text scaling, insets and keyboard overlap; light-mode device captures. Targeted checks + maintainer/device visual acceptance; recent styling commits are not a completed global audit. |
