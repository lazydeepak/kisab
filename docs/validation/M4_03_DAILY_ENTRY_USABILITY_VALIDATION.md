...
### Instrumentation (`:app:connectedDebugAndroidTest`) — 57 tests, 0 failures

57 tests, zero failures, zero unexpected skips, and no crash or ANR:

| Device | ABI | API | Full suite | Result |
| --- | --- | --- | --- | --- |
| Pixel 7a (physical, USB) | arm64-v8a | 37 | 57 | pass |

Full-suite confirmation runs on the Pixel 7a. (The API 26 and API 36 emulators previously carried the 54-test suite green; the emulator API 26 host became unstable with a launcher ANR and was replaced by Pixel-only connected runs per operator instruction. `FarmBackupIntegrationTest` additionally passed 8/8 on the API 36 emulator.)

`FarmBackupIntegrationTest` (8 tests) now directly verifies the active-editor backup-replacement state machine through `handleImportedBackupContent` with deterministic backup content (a `FarmState` built with a distinct farm id and transaction id, encoded via `FarmBackupCodec.encode`): (1) dirty editor + **Keep editing** — the discard dialog's keep-editing button leaves the original farm persisted, the replacement farm unsaved, the editor visible with the unsaved amount and description intact, and the expense type/category/currency unchanged, with no transaction created; (2) dirty editor + **Discard** — the replacement farm is persisted and becomes current, the old farm is no longer active, the editor and its validation state are cleared, recent rows and totals belong only to the restored farm, the old unsaved description is absent, and a stale-ID check re-opens the restored transaction, confirms its populated values belong to the restored farm, and updates it by its restored id without reusing the original transaction id; (3) clean editor + **Replace farm** — no discard dialog appears, the replacement completes, the editor closes, the restored farm is active, and no blank transaction is created. Existing backup coverage (export/decode, restore, cancellation, invalid and unsupported rejection, immediate recreation, schema 1) is unchanged.
`FarmActivityWorkflowTest` (20 tests) covers the daily-entry workflow: first income with no currency/ISO input and default-now time; repeated expense derives currency and supplies current time; edit preserves transaction identity; edit without date change preserves the exact instant; recreation preserves the editor draft; cancel and back require discard confirmation; switching to another transaction while dirty confirms; record while dirty confirms; empty-farm NPR default with currency choice and post-save lock; established farm currency lock; sole-transaction currency change; human-readable date/time with native picker (no ISO typing); farm-tools expansion/collapse; save label follows the selected transaction type (income/expense); invalid ISO currency keeps the dialog open with an inline error; dirty/clean editor baseline survives activity recreation (discard protection and silent close respectively); farm-tools expansion survives recreation; and currency fraction digits apply per ISO (`JPY` 0, `KWD` 3). The editor date/time is always entered through the real `DatePickerDialog`/`TimePickerDialog` (`PickerTestHelpers.pickDateTime`, device zone, asserted via `expectedInstant`), and the currency chooser through its text-input dialog (`selectCurrency` types the ISO code into `currencyInput` and confirms); the previous test-only `setOccurredAt`/`overrideEditorOccurredAtForTest` seams were removed. Existing suites (`FarmActivityPresentationTest`, `FarmPersistenceIntegrationTest`, `FarmActivityLocalizationSmokeTest`, `LocalizedResourceResolutionTest`, session tests) were migrated to the new views and key set (e.g. `farm_tools_summary_format` with 3 args, `recentTransactionsContainer` rows) and pass unchanged in behavior. No sleeps/retries; dialog interactions are root-scoped (`inRoot(isDialog())`); locales and store state are cleared and restored between tests; every `ActivityScenario` is closed. One intermittent load-related flake observed on the `api26` emulator (currency selection showing the prior ISO after a full parallel run) was confirmed non-reproducible in isolation and across a full-suite re-run on all three devices.
